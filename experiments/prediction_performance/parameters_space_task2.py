...
p['data_path']['parameterspace_label'] = 'prediction_performance_task2'

# parameters for setting up the network  
p['M'] = 14                  # number of subpopulations
...
p['syn_dict_ee']['lambda_plus'] = 0.28               # potentiation rate
p['syn_dict_ee']['zt'] = 1.                          # target dAP trace
p['syn_dict_ee']['lambda_h'] = 0.024                 # homeostasis rate
...
p['syn_dict_ee']['lambda_minus'] = 0.0063            # depression rate: in (Bouhadjar et al. 2022, PloS CB), we indicate that we use 0.0061. Use rather 0.0063. Note that we have not done any extensive parameter search, it may be possible to find set of parameters that can give even better prediction performance (in term of episode to solution) than the one presented in (Bouhadjar et al. 2022, PloS CB). Future works will closely study this point.
p['syn_dict_ee']['dt_min'] = -4.
...
p['active_weight_recorder'] = False
...
p['record_idend_last_episode'] = False         # used for debugging, if turned on we record the dendritic current of all neurons
...
p['task']['task_type'] = 2                     # this chooses between three hard coded sequence sets (see ./utils.py)
p['task']['vocab_size'] = 14                    # vocabulary size
