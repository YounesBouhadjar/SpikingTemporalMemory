...
#    else:
    ...
    if params['evaluate_performance']:
        ...
        sequences_overlap = helper.measure_sequences_overlap(model_instance.sequences,
                                                         somatic_spikes[:,1],
                                                         somatic_spikes[:,0],
                                                         excitation_times,
                                                         params['fixed_somatic_delay'],
        ...
        overlap = [ov[-2] for ov in sequences_overlap]

        print("Overlap of the element before last across training:\n", overlap)

        wandb.log({"loss"+str(arr_id): seq_avg_errors[-1],
                   "fp"+str(arr_id): seq_avg_false_positives[-1],
                   "fn"+str(arr_id): seq_avg_false_negatives[-1],
                   "overlap_last"+str(arr_id): overlap[-1]})

        xs = np.arange(len(overlap))
        data = [[x, y] for (x, y) in zip(xs, overlap)]
        table = wandb.Table(data=data, columns=["x", "y"])

        wandb.log({"overlap_time": wandb.plot.line(table, "x", "y")})
